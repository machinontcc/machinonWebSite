...
function gerarPDF(report, startDate, endDate) {
    const { jsPDF } = window.jspdf;  // Usando o jsPDF do CDN
    const doc = new jsPDF();  // Cria uma nova instância do jsPDF

    // Definindo a fonte (tamanho menor e uma fonte mais adequada)
    doc.setFont('helvetica');
    doc.setFontSize(10);

    // Logo da empresa (supondo que o logo da empresa esteja em uma URL)
    ...
    doc.addImage(logoBase64, 'PNG', 10, 10, 40, 20); // Ajuste a posição e o tamanho conforme necessário
    ...
    doc.line(10, 35, 200, 35); // Linha para separar a parte superior do conteúdo

    // Adicionando as estatísticas de funcionários
    doc.setFontSize(12);
    doc.setFont('helvetica', 'bold');
    ...
    // Adicionando as leituras dos sensores
    doc.setFont('helvetica', 'bold');
    doc.text('Leituras dos Sensores:', 20, 105);
    let yPosition = 115; // Começar a partir de 115 para as leituras de sensores
